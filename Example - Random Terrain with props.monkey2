...
Class MyWindow Extends Window
	...
	Method startgame()
    	...
    	Local distance:=Lambda:Int(x1:Int,y1:Int,x2:Int,y2:Int)
        	Return Abs(x2-x1)+Abs(y2-y1)
    	End Lambda

		'here we place the buildings(or things)		
		Local myx:=New Stack<Int>
		Local myy:=New Stack<Int>		
		Local placedcnt:Int=0
		For Local i:=0 Until 500
			Local x:Int=Rnd(mapsize)
			Local y:Int=Rnd(mapsize)
			
			' Find surface that is suitable (flat area)
			Local yes:Bool=True
			For Local y2:=y-5 Until y+5
			For Local x2:=x-5 Until x+5
				If x2>=0 And x2<mapsize And y2>=0 And y2<mapsize
					If heightMap.GetPixel(x2,y2).r < .27 or heightMap.GetPixel(x2,y2).r > .33
						yes=False
					End If
				Else
					yes=False
			...
			Next
			Next
			' if we have found a flat area
			Local skippit:Bool=False
			If yes
				placedcnt+=1
				' Print "hello"
				For Local i2:=0 Until myx.Length
					If distance(x,y,myx.Get(i2),myy.Get(i2)) < 20 Then skippit=True
				Next
				If skippit = False Then
				Local mymesh := Mesh.CreateBox( New Boxf( -10,-35,-10,10,35,10 ),1,1,1 )
				Local material:=New PbrMaterial( New Color( Rnd(0.0,0.6),0,0) )
				Local model:=New Model( mymesh,material )
				Local x1:Int=-mapsize
				Local y1:Int=-mapsize
				'It appears that to read the heightmap(pixmap) you need
				' to use the move as -the mapsize and then the pixel position
				' times 2.
				model.Move(x1+(x*2),0,y1+(y*2))
				myx.Push(x)
				myy.Push(y)
				End If
				
			End If
			' if more then 10 buildings placed then exit.
			If placedcnt > 10 Then Exit
		Next
